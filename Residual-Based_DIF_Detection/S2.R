############################################################
## Residual-Based DIF Simulation Script: Study 2
## Follow-up stress test informed by Study 1
##
## Revised design:
##   - two-group only: reference vs focal
##   - data-generation model fixed to the 2PLM
##   - sample-size schemes: 1000R/1000F and 1600R/400F
##   - 20 dichotomous items
##   - three item-difficulty profiles:
##       centered, moderately hard, and hard
##   - Woods-style skewed latent ability distributions
##   - nonlinear/crossing nonuniform DIF
##   - DIF proportion fixed at 20% (4 of 20 items)
##   - two nonlinear DIF magnitudes: gamma = 0.50 and 1.00
##   - purification via purify.by = "rdifrs"
############################################################
...
item_difficulty_profiles <- tibble::tribble(
  ~item_profile,       ~b_mean,
  "centered",            0.00,
  "moderately_hard",     0.75,
  "hard",                1.25
)
...
impact_conditions <- c(
  ...
  "mean_impact",
  "focal_only_skewness",
  "reference_only_skewness"
)

dif_magnitude_levels <- tibble::tribble(
  ~DIF_magnitude, ~DIF_gamma,
  "moderate",        0.50,
  "large",           1.00
)
...
make_design <- function(reps_per = 100) {
  base <- tidyr::expand_grid(
    ...
    profile_tbl = seq_len(nrow(item_difficulty_profiles)),
    impact      = impact_conditions,
    mag_tbl     = seq_len(nrow(dif_magnitude_levels)),
    ...
    dplyr::mutate(
      ...
      item_profile = item_difficulty_profiles$item_profile[profile_tbl],
      b_mean       = item_difficulty_profiles$b_mean[profile_tbl],
      n_items      = 20,
      DIF_pct      = 20,
      DIF_type     = "nonlinear_crossing",
      DIF_magnitude = dif_magnitude_levels$DIF_magnitude[mag_tbl],
      DIF_gamma     = dif_magnitude_levels$DIF_gamma[mag_tbl]
    ) |>
    dplyr::select(-scheme_tbl, -profile_tbl, -mag_tbl) |>
    dplyr::mutate(
      cond_id = dplyr::row_number(),
      seed    = 200000 + cond_id,

      ## Use the same base random components across the two gamma levels
      ## within the same sample-size scheme, item-difficulty profile,
      ## impact/distribution condition, and replication.
      base_key = paste(scheme, item_profile, impact, rep, sep = "_"),
      base_id  = as.integer(factor(base_key)),
      item_seed = 300000 + base_id,
      sim_seed  = 400000 + base_id
    ) |>
    dplyr::select(-base_key, -base_id)
...
## Study 2 uses 20-item tests.
## a ~ LN(.5, .3^2) truncated to [1.0, 3.5]
## b is generated from one of three item-difficulty profiles:
##   centered:        N(0.00, 1), truncated to [-2.5, 2.5]
##   moderately hard: N(0.75, 1), truncated to [-2.5, 2.5]
##   hard:            N(1.25, 1), truncated to [-2.5, 2.5]

gen_reference_item_bank <- function(J = 20,
                                    model = "2PLM",
                                    b_mean = 0,
                                    b_sd = 1,
                                    b_range = c(-2.5, 2.5)) {
  if (!identical(model, "2PLM")) {
    stop("Study 2 is fixed to the 2PLM.")
  }

  a <- rtrunc_lnorm(J, meanlog = 0.5, sdlog = 0.3, lower = 1.0, upper = 3.5)
  b <- rtrunc_norm(J, mean = b_mean, sd = b_sd, lower = b_range[1], upper = b_range[2])
  g <- rep(0, J)

  list(a = a, b = b, g = g, original_item = seq_len(J))
}

select_dif_items <- function(J, DIF_pct = 20) {
  j_dif <- round(J * DIF_pct / 100)
  dif_idx <- if (j_dif > 0) sort(sample(seq_len(J), j_dif)) else integer(0)

  if (j_dif > 0) {
    signs <- rep(c(1, -1), length.out = j_dif)
    signs <- sample(signs, length(signs))
    names(signs) <- dif_idx
  } else {
    signs <- numeric(0)
  }

  list(dif_idx = dif_idx, signs = signs)
}

shape_item_df <- function(bank, model = "2PLM") {
  if (!identical(model, "2PLM")) {
    stop("Study 2 is fixed to the 2PLM.")
  }

  irtQ::shape_df(
    par.drm = list(a = bank$a, b = bank$b, g = rep(0, length(bank$a))),
...
## Woods-style skewed latent distribution:
## theta^S ~ .897 N(-.253, .609^2) + .103 N(2.192, 1.045^2)
## This mixture has approximately mean 0 and variance 1 with positive skewness.

rwoods_skewed <- function(n) {
  main_comp <- stats::runif(n) <= 0.897
  theta <- numeric(n)
  theta[main_comp] <- stats::rnorm(sum(main_comp), mean = -0.253, sd = 0.609)
  theta[!main_comp] <- stats::rnorm(sum(!main_comp), mean = 2.192, sd = 1.045)
  theta
}

sim_abilities_s2 <- function(n_ref, n_focal,
                             impact = c(
                               "no_impact",
                               "mean_impact",
                               "focal_only_skewness",
  ...
  impact <- match.arg(impact)

  if (impact == "no_impact") {
    theta_ref   <- rwoods_skewed(n_ref)
    theta_focal <- rwoods_skewed(n_focal)
  } else if (impact == "mean_impact") {
    theta_ref   <- rwoods_skewed(n_ref)
    theta_focal <- rwoods_skewed(n_focal) - 0.6
  } else if (impact == "focal_only_skewness") {
    theta_ref   <- stats::rnorm(n_ref, mean = 0, sd = 1)
    theta_focal <- rwoods_skewed(n_focal)
  } else if (impact == "reference_only_skewness") {
    theta_ref   <- rwoods_skewed(n_ref)
    theta_focal <- stats::rnorm(n_focal, mean = 0, sd = 1)
  }

  list(theta_ref = theta_ref, theta_focal = theta_focal)
}

nonlinear_crossing_h <- function(theta, b) {
  x <- theta - b
  x * exp(-0.5 * x^2)
}

simulate_responses_2pl <- function(bank, theta,
                                   dif_idx = integer(0),
                                   dif_signs = numeric(0),
                                   nonlinear = FALSE,
                                   gamma = 0.50,
  ...
  N <- length(theta)
  J <- length(bank$a)
  P <- matrix(NA_real_, nrow = N, ncol = J)

  for (j in seq_len(J)) {
    eta <- D * bank$a[j] * (theta - bank$b[j])

    if (nonlinear && j %in% dif_idx) {
      s_j <- as.numeric(dif_signs[as.character(j)])
      if (!is.finite(s_j)) s_j <- 1
      eta <- eta + s_j * gamma * nonlinear_crossing_h(theta = theta, b = bank$b[j])
    }
  ...
  resp <- matrix(
    stats::rbinom(N * J, size = 1, prob = as.vector(P)),
    nrow = N,
    ncol = J
  )
...
simulate_two_group_data <- function(n_ref, n_focal,
                                    model = "2PLM",
                                    n_items = 20,
                                    b_mean = 0,
                                    DIF_pct = 20,
  ...
  if (!identical(DIF_type, "nonlinear_crossing")) {
    stop("Study 2 uses nonlinear/crossing nonuniform DIF only.")
  }
  impact <- match.arg(impact)

  if (!is.null(item_seed)) set.seed(item_seed)
  bank <- gen_reference_item_bank(J = n_items, model = model, b_mean = b_mean)

  ## Use a condition-specific seed after the item parameters have been generated.
  if (!is.null(sim_seed)) set.seed(sim_seed)

  dif_info <- select_dif_items(J = length(bank$a), DIF_pct = DIF_pct)
  ...
  resp_ref <- simulate_responses_2pl(
    bank = bank,
    theta = th$theta_ref,
    nonlinear = FALSE,
    D = D
  ...
  resp_focal <- simulate_responses_2pl(
    bank = bank,
    theta = th$theta_focal,
    dif_idx = dif_info$dif_idx,
    dif_signs = dif_info$signs,
    nonlinear = TRUE,
    gamma = DIF_gamma,
    D = D
  )
  ...
  list(
    ...
    ref_bank   = bank,
    focal_bank = bank,
    original_item = bank$original_item,
...
make_empty_rdif_tab <- function(J, alpha = 0.05) {
  tibble::tibble(
    item = seq_len(J),
    item_id = paste0("I", seq_len(J)),
    rdifr = NA_real_,
    z_rdifr = NA_real_,
    rdifs = NA_real_,
    z_rdifs = NA_real_,
    ...
    n_total = NA_real_,
    iter = NA_integer_,
    flagged_rdifr = rep(FALSE, J),
    flagged_rdifs = rep(FALSE, J),
    flagged_rdifrs = rep(FALSE, J),
    rdifr_p_was_na = NA,
    rdifs_p_was_na = NA,
    rdifrs_p_was_na = NA
  )
...
run_one_condition <- function(cond,
  ...
  dif_sign <- rep(0, J)
  if (length(sim$dif_idx) > 0) {
    dif_sign[sim$dif_idx] <- as.numeric(sim$dif_signs[as.character(sim$dif_idx)])
  }
  ...
  semi_res <- tryCatch(
    ...
    error = function(e) {
      ...
      append_log(
        sprintf("Spline-LR failed at cond_id=%s: %s", cond$cond_id, e$message),
        logfile = logfile
      )
  ...
  method_error_message <- paste(
    na.omit(c(
      ...
      if (!is.na(semi_error)) paste0("Spline-LR: ", semi_error),
  ...
  out <- rdif_tab |>
    ...
    dplyr::mutate(
      y_true = y_true,
      dif_sign = dif_sign,
      original_item = sim$original_item[item],

      model = cond$model,
      scheme = cond$scheme,
      n_ref = cond$n_ref,
      n_focal = cond$n_focal,
      item_profile = cond$item_profile,
      b_mean = cond$b_mean,
      n_items = cond$n_items,
      DIF_pct = cond$DIF_pct,
      DIF_type = cond$DIF_type,
...
make_error_result <- function(cond, error_message) {
  tibble::tibble(
    ...
    semi_n_skipped_final = NA_integer_,
    semi_n_anchor_final = NA_integer_,
    semi_converged = NA,
    ...
    mi_n_skipped_final = NA_integer_,
    mi_n_anchor_final = NA_integer_,
    mi_converged = NA,
...
run_full_grid <- function(grid,
  ...
  purrr::map_dfr(seq_len(nrow(grid)), function(i) {
    ...
    cat(sprintf(
      "[%d/%d] cond_id=%s | scheme=%s | profile=%s | impact=%s | gamma=%s | rep=%s\n",
      i, nrow(grid), cond$cond_id, cond$scheme, cond$item_profile,
      cond$impact, cond$DIF_gamma, cond$rep
    ))
    flush.console()
...
run_full_grid_resume <- function(grid,
                                 out_dir = "sim_chunks_s2_purified_v2",
                                 alpha = 0.05,
                                 D = 1,
                                 purify = TRUE,
                                 purify.by = "rdifrs",
  ...
  for (i in seq_len(nrow(grid_todo))) {
    ...
    cat(sprintf(
      "[%d/%d remaining] cond_id=%s | scheme=%s | profile=%s | impact=%s | gamma=%s | rep=%s\n",
      i, nrow(grid_todo), cond$cond_id, cond$scheme, cond$item_profile,
      cond$impact, cond$DIF_gamma, cond$rep
    ))
    flush.console()
    ...
    append_log(
      sprintf(
        "Starting cond_id=%s | scheme=%s | profile=%s | impact=%s | gamma=%s | rep=%s",
        cond$cond_id, cond$scheme, cond$item_profile,
        cond$impact, cond$DIF_gamma, cond$rep
      ),
      logfile = logfile
    )
...
collect_results <- function(out_dir = "sim_chunks_s2_purified_v2") {
...
check_progress <- function(grid, out_dir = "sim_chunks_s2_purified_v2") {
...
collect_errors <- function(res) {
  res |>
    ...
    dplyr::distinct(
      cond_id, model, scheme, item_profile, b_mean, n_items,
      DIF_pct, DIF_type, DIF_magnitude, DIF_gamma, impact, rep,
      error_message
    )
}
...
summarise_ml_finiteness <- function(res) {
  res |>
    dplyr::distinct(
      cond_id, model, scheme, item_profile, b_mean, DIF_pct,
      DIF_type, DIF_magnitude, DIF_gamma, impact, rep,
      status,
      ml_all_theta_finite,
      ml_n_na_theta,
      ml_n_infinite_theta,
...
summarise_by_replication <- function(res, grid = NULL) {
  ...
  res_cond <- res |>
    dplyr::distinct(
      ...
      item_profile, b_mean, n_items, DIF_pct, DIF_type,
      DIF_magnitude, DIF_gamma, impact, rep,
  ...
  if (is.null(grid)) {
    out <- res_cond |>
      dplyr::left_join(perf_usable, by = join_keys)
  } else {
    out <- grid |>
...
summarise_perf_mean <- function(perf_all) {
  perf_all |>
    dplyr::group_by(
      model, scheme, n_ref, n_focal,
      item_profile, b_mean, n_items, DIF_pct, DIF_type,
      DIF_magnitude, DIF_gamma, impact
    ) |>
    dplyr::summarise(
      ...
      dplyr::across(
        where(is.numeric) &
          !dplyr::any_of(c(
            "cond_id", "n_ref", "n_focal", "n_items",
            "b_mean", "DIF_pct", "DIF_gamma", "rep", "seed", "item_seed", "sim_seed",
...
## Full Study 2: reps_per = 100, yielding 4,800 conditions.
...
perf_mean <- summarise_perf_mean(perf_all)

## Check whether all 4,800 conditions are retained.
nrow(perf_all)
